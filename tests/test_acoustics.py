...
def test_only_const():
    ...
    f = [0]*4
    f[1] = drive.get_local_acoustics(0, [0,0,0], [1,1], [1,0,0]).frequency
    f[2] = drive.get_local_acoustics(10, [0,0,0], [1,1], [1,0,0]).frequency
    f[3] = drive.get_local_acoustics(10, [1,1,1], [1,1], [1,0,0]).frequency
    f[0] = drive.get_local_acoustics(10, [1,1,1], [1,1], [0,0,1]).frequency
    
    assert allclose(f,f[0])

def test_no_offset():
    drive = AcousticDrive(10, 100)
    f = drive.get_local_acoustics(0, [2,3,1], [2,3], [1,0,0]).frequency
    assert f == 10
...
def test_t_shift():
    ...
    f1 = drive.get_local_acoustics(0, [0,0,0], [0,0], [1,0,0]).frequency
    f2 = drive.get_local_acoustics(10, [0,0,0], [0,0], [1,0,0]).frequency
