...
from ray import Ray
import ray_paraxial as paraxial
...
def test_ray_passes_through_focus():
    ...
    ray1 = paraxial.Ray([0,0,0], [0,0,1], op_wavelength)
    aol.propagate_to_distance_from_aol(ray1, 0, focus_position[2]) # t = 0
    
    ray2 = paraxial.Ray([0,0,0], [0,0,1], op_wavelength)
    aol.propagate_to_distance_from_aol(ray2, 11, focus_position[2]) # t != 0
    
    ray3 = paraxial.Ray([1,0,0], [0,0,1], op_wavelength) # position not origin
