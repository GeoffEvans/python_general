...
def diffract_by_wavevector_triangle(aod, ray, local_acoustics):
    ...
    def zero_func(mismatch):
        wavevector_mismatch = mismatch * aod.normal * 1e6
        ...
        wavevector_out_mag2 = n_ord * wavevector_vac_mag_out # pretty much constant
        
        return wavevector_out_mag2 - wavevector_out_mag1 

    wavevector_mismatch_mag = fsolve(zero_func, 0) * 1e6
    ...
    ray.wavevector_unit = normalise(wavevector_out)
    return  (wavevector_mismatch_mag, original_ray) # return phase mismatch

def get_aod_efficiency(aod, wavevector_mismatch_mag, ray_in, ray_out, acoustics):
    ...
    delta_n0 = -0.5 * n_in**2 * n_out * p * acoustics.amplitude(aod) # Xu&St (2.128)
    delta_n1 = -0.5 * n_out**2 * n_in * p * acoustics.amplitude(aod)
