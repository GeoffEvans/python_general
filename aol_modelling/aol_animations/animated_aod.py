...
import aol_model.acoustics as a

pointing_ramp_time = 1e-5
...
for time in np.linspace(0, pointing_ramp_time*10, num_times)[:-1]:
    stationary_acoustics = True
    t = time
    if stationary_acoustics:
        t = 0
    ...
    local_acoustics = drive.get_local_acoustics(t, [r.position for r in rays], [0,0], aod.acoustic_direction)
    ...
    local_acoustics2 = drive.get_local_acoustics(t, [[x,0] for x in x_list], [0,0], aod.acoustic_direction)
